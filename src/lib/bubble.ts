...
/**
 * Fetch all bubble_ids from a Bubble object type
 */
async function fetchAllBubbleIds(typeName: string): Promise<Set<string>> {
 ...
 * 2. Syncs all submit_payment records from Bubble (overwrites local data)
 * 3. DELETES submitted_payments that no longer exist in Bubble (verified payments)
 * 4. For NEW payments detected, traces linked_invoice and syncs full invoice package
 *
  ...
  try {
    ...
    for (const inv of bubbleInvoices) {
      ...
      try {
        ...
        logSyncActivity(`Warning: Could not check items for invoice ${inv._id}: ${err}`, 'INFO');
      }
